...
class PlatformMetaTest:
    ...
    def failed(self, message: str = "") -> None:
        ...
        if message:
            print(message)
        print(f"{self.script_name} failed!")
        sys.exit(1)
    ...
    def check_somerville_xenial(self, biosid: str) -> None:
        ...
        for line in stdout.split('\n'):
            ...
            if len(parts) >= 2 and '-meta' in parts[1]:
                meta = parts[1]
                # Check pattern: dell-BIOSID(-xenial)?-meta
                pattern = f'^dell-{biosid}(-xenial)?-meta$'
                if re.match(pattern, meta):
                    # Get dependencies
                    ...
                    for dep_line in dep_stdout.split('\n'):
                        if 'Depends:' in dep_line:
                            ...
                            if '-meta' in pmeta:
                                self.passed(f"Found platform meta packages: {pmeta}, {meta}")
    ...
    def check_somerville_bionic(self, biosid: str) -> None:
        ...
        for line in stdout.split('\n'):
            ...
            if len(parts) >= 2 and '-meta' in parts[1]:
                meta = parts[1]
                pattern = f'^dell-{biosid}(-bionic)?-meta$'
                if re.match(pattern, meta):
                    dep_stdout, _ = self.run_command(['apt-cache', 'depends', meta])
                    for dep_line in dep_stdout.split('\n'):
                        if 'Depends:' in dep_line:
        ...
        if self.platform_tag:
            ...
            if self.is_package_installed(pmeta):
                if self.has_modalias_match(pmeta, f'sv00001028sd0000{biosid}'):
                    self.passed(f"Found platform meta package '{pmeta}' containing BIOS ID '{biosid}'")
    ...
    def check_somerville_meta(self) -> None:
        ...
        platform_info = f"Platform Tag: {self.platform_tag}, " if self.platform_tag else ""
    ...
    def run(self) -> None:
        ...
        if self.oem == 'somerville':
            ...
            print(f"Unknown OEM codename: {self.oem}", file=sys.stderr)
            sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        '--platform-codename',
        help='Platform codename (optional)'
    )
